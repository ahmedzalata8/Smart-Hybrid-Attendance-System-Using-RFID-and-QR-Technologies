#!/usr/bin/env python3
"""
RFID Reader Capture Script for R16-12DB UHF RFID Reader

Production-quality script for reading RFID tags from an R16-12DB UHF RFID reader
via USB serial connection. Supports auto-detection, frame parsing, RSSI extraction,
...
def select_port(preferred_port: Optional[str] = None) -> Optional[str]:
    ...
    if preferred_port:
        ...
        # Try to verify by checking if it's in the list or just trust the user
...
class RFIDReader:
    ...
    def __init__(
        ...
        # Track which seats have been scanned in this session
        self.scanned_seats = set()
        
        # Track consecutive tag reads to detect when reader is stuck on old tag
        self.last_tag_id = None
        self.last_tag_time = 0
        self.tag_read_count = 0
        self.tag_first_seen = 0
        self.tag_is_stale = False  # Once marked stale, stay stale until long gap
        self.stale_tag_threshold = 1  # After 1 read, mark as stale (only accept first detection)
        self.stale_clear_timeout = 15.0  # Need 15 second gap to clear stale status
        self.tag_presence_timeout = 15.0  # After 15 seconds of no activity, accept new reads
    ...
    def disconnect(self) -> None:
        ...
        # Reset stale tag tracking
        self.last_tag_id = None
        self.last_tag_time = 0
        self.tag_read_count = 0
        self.tag_first_seen = 0
        self.tag_is_stale = False
    ...
    def is_stale_tag(self, tag_id: str, current_time: float) -> bool:
        """Check if tag is stale (reader stuck transmitting old tag)"""
        if not tag_id:
            return False
        ...
        if current_time - self.startup_time < self.startup_grace_period:
            if self.debug and int(current_time * 10) % 20 == 0:  # Log occasionally
                logging.debug(f"Startup grace period - ignoring data")
            return True
        
        # Normalize tag ID by finding the most common pattern
        # The reader outputs the same tag in rotating chunks
        normalized_tag = self._normalize_tag_id(tag_id)
        
        # Check if we need to clear stale status after long silence
        if self.last_tag_time > 0:
            time_since_last_read = current_time - self.last_tag_time
            if time_since_last_read >= self.stale_clear_timeout:
                # Long silence - clear stale status and allow new reads
                if self.debug:
                    logging.debug(f"Clearing stale status after {int(time_since_last_read)}s gap - ready for new scan")
                self.tag_is_stale = False
                self.last_tag_id = None
                self.tag_read_count = 0
        
        # If already marked as stale, stay stale until cleared
        if self.tag_is_stale and self.last_tag_id == normalized_tag:
            if self.debug and self.tag_read_count % 500 == 0:
                logging.debug(f"Still filtering stale tag ({self.tag_read_count} reads filtered)")
            self.tag_read_count += 1
            self.last_tag_time = current_time
            return True
        
        if self.last_tag_id == normalized_tag:
            # Same tag as before
            self.tag_read_count += 1
            
            # Mark as stale immediately after first accepted read
            if self.tag_read_count > self.stale_tag_threshold:
                if not self.tag_is_stale and self.debug:
                    logging.debug(f"Tag marked as STALE after {self.tag_read_count} reads - will ignore until 15s gap")
                self.tag_is_stale = True
                self.last_tag_time = current_time
                return True
        else:
            # Different tag detected!
            time_since_last_seen = current_time - self.last_tag_time if self.last_tag_time > 0 else float('inf')
            
            # For a DIFFERENT tag, always accept it (it's a new scan)
            # Only require gap time if it's the SAME tag being re-scanned
            if self.debug and self.last_tag_id:
                logging.debug(f"Different tag detected - accepting new scan (last tag was {self.last_tag_id[:20]}...)")
            
            self.last_tag_id = normalized_tag
            self.tag_first_seen = current_time
            self.last_tag_time = current_time
            self.tag_read_count = 1
            self.tag_is_stale = False
    ...
    def process_data(self, data: bytes) -> None:
        ...
        if tag_id:
            ...
            if self.is_stale_tag(tag_id, current_time):
                ...
                if self.debug and self.tag_read_count == self.stale_tag_threshold:
                    logging.debug(f"Stale tag detected (same tag for {time.time() - self.last_tag_time:.1f}s) - ignoring until gap")
                return
    ...
    def _process_tag(self, tag_id: str, rssi: Optional[int], raw_hex: str) -> None:
        ...
        # Check if this seat was already scanned in this session
        if seat_id in self.scanned_seats:
            self.stats['already_scanned_filtered'] += 1
            if self.debug:
                logging.debug(f"Seat {seat_id} already scanned in this session - ignoring")
            return
        
        # Mark this seat as scanned
        ...
        print(f"[{timestamp_iso}] SeatID={seat_id} TagID={tag_id} RSSI={rssi_str} Port={self.port}")
...
Examples:
  ...
  %(prog)s --dedupe-ms 1000 --baud 9600
    ...
    # Banner
    print("="*60)
    print("RFID Reader Capture - R16-12DB UHF RFID Reader")
    print("="*60)
