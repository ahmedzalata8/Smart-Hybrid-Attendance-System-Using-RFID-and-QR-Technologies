...
async def get_digital_twin(
    ...
    # Fetch attendance records for this session. A seat can have more than one
    # record (different students across attempts) — prefer the PRESENT one so a
    # claimed+occupied seat is the one shown on the twin.
    att_result = await db.execute(
        select(AttendanceRecord).where(AttendanceRecord.session_id == session_id)
    )
    att_records: dict = {}
    for r in att_result.scalars().all():
        cur = att_records.get(r.seat_id)
        if cur is None or r.status.value == "present":
            att_records[r.seat_id] = r

    # Resolve student names for those records (for the digital-twin labels).
    student_ids = {r.student_id for r in att_records.values()}
    student_names: dict = {}
    if student_ids:
        users_result = await db.execute(select(User).where(User.id.in_(student_ids)))
        student_names = {u.id: u.full_name for u in users_result.scalars().all()}
    ...
    for seat_state, seat in seat_state_rows:
        ...
        seats_out.append(SeatStateOut(
            ...
            student_name=student_names.get(att_record.student_id) if att_record else None,
